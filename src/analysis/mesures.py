...
def s_recallMeasure(queryResCat, queryCatRes, grontoResCat, grontoCatRes):
    srOnto = 0.0
    srExp = 0.0
    ...
    res_granules= set()
    for res in grontoResCat:
        for c in grontoResCat[res]:
            res_granules.add( c )
    srOnto = float(len(res_granules))/162

    res_granules= set()
    for res in queryResCat:
       for c in queryResCat[res]:
           res_granules.add( c )
           
    srExp = float(len(res_granules))/219
...
def main(argv):
    ...
    for root, dirs, files in os.walk(sampleDir):
        ...
        for t in sampleTag:
            ...
            g = os.path.join(root, t + "_NOGRANULE_repetition.txt")
            q = os.path.join(root, t + "_query.txt")
            s = os.path.join(root, t + "_sorting.txt")
            
            # load data
            qRes, qCat, gRes, gCat = loadSample(q, g, s, cutAt=20)
            ...
            m5a, m5b = s_recallMeasure(qRes, qCat, gRes, gCat)
            
            # put here other measures ... //TODO
            measures[tagId] = [m1, m2, m3, m4, m5a, m5b]
    ...
    for t in measures:
        ...
        avgRec += measures[t][3]
        avgSRecOnto += measures[t][4]
        avgSRecExpert += measures[t][5]
    ...
    avgSRecOnto /= len(measures)    
    avgSRecExpert /= len(measures)    
    ...
    print "#average: coverage=%f, ndcg=%f, prec=%f, rec=%f, s-rec=%s"%(avgCov, avgNdcg, avgPrec,
                                                                       avgRec, str((avgSRecOnto,avgSRecExpert)))
